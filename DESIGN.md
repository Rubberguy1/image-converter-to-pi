...
faceplate label. Small sizes are the norm; hierarchy comes from weight and color,
not from large display type. There is no display/hero tier — this surface never
shouts.

### Hierarchy
- **Heading** (600, 1rem, 1.3): Section titles (`settings-section h3`), panel
  headers. The largest routine type; carried by weight, not size.
- **Body** (400, 0.85rem, 1.4): Default text, button labels, values.
- **Label** (500, 0.7rem, +0.02em, sometimes UPPERCASE): Field labels, hints,
  source tags, badges. The quiet silk-screen layer.
- **Readout / Mono** (400, 0.72rem): Live machine state only — Hz, ms, watts,
  CPU %, player counts, versions. Monospace signals "this is a live reading."
...
### Don't:
- **Don't** hand-pick near-duplicate hexes. `#4ea1ff`, `#5fd08a`, and `#ff7a7a` have crept into recent panels (notifications, game servers) — they must migrate to `--accent` / `--accent-2` / `--danger`. Off-token color erodes the LED Rule.
